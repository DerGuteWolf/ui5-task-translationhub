...
module.exports = async function({workspace, dependencies, taskUtil, options}) {
    ...
    for (let propertyFile of propertyFiles) {
        let asyncWorkForContent = async () => {
            ...
            data.append('file', await propertyFile.getBuffer(),  { filename: `${pathName.replace('/', '_')}i18n.properties`, contentType: 'application/octet-stream' }); //propertyFile.getStream()); // gives Content of Resource /i18n/i18n.properties has been drained
            try {
                // upload file
